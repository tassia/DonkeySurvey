...
class AddressHasFileDAO(CommonDAO):
    ...
    def insertOrUpdate(self, addressId, fileId, firstSeen):
        ...
        if not rs:
            query = "INSERT INTO %s(address_id, file_id, first_seen) VALUES(%d, %d, '%s')" % (self.jointable, addressId, fileId, firstSeen)
            logging.debug(query)
            self.cursor.execute(query)
        #CommonDAO.lastID(self, self.jointable)

    def delete(self, addressId, fileId):
        self.cursor.execute("""DELETE FROM """+self.jointable+""" WHERE address_id = %s AND file_id = %s""", (addressId, fileId))

    def sourceHasFile(self, addressId, fileId):
        self.cursor.execute("""SELECT * FROM """+self.jointable+""" WHERE address_id = %s AND file_id = %s""", (addressId, fileId))
